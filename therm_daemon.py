...
#set working directory to where "therm_daemon.py" is
...
class thermDaemon(Daemon):
    ...
    def run(self):
        ...
        self.configureGPIO()
        #change cwd to wherever therm_daemon is
        abspath = os.path.abspath(__file__)
        dname = os.path.dirname(abspath)
        os.chdir(dname)
        ...
        while True:
            ...
            if DEBUG == 1:
                log = open("logs/debug_" + now.strftime('%Y%m%d') + ".log", "a")
...
if __name__ == "__main__":
        daemon = thermDaemon('thermDaemon.pid')
